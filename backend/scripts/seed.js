...
async function seed() {
  try {
    ...
    // 2. Admin account
    const adminPass = await bcrypt.hash("anhle123456", 10);
    console.log(adminPass);
    await db.query(
      `
      INSERT IGNORE INTO User (id, role_id, full_name, email, password, status)
      VALUES (1, 1, 'Quản trị viên', 'anhanhle1997@gmail.com', ?, 'active')
    `,
      [adminPass],
    );
    console.log(
      "✅ Admin seeded if missing  → expected id: 1 | role_id: 1 | email: anhanhle1997@gmail.com | password: anhle123456",
    );
    ...
    console.log(
      "✅ User seeded   → email: user@lunexa.vn   | password: User@123",
    );
