...
export default function MovieDetail() {
  ...
  const galleryImages = [
    posterSrc,
    ...(Array.isArray(movie?.posters) ? movie.posters : []),
  ].filter(Boolean);
  ...
  useEffect(() => {
    if (hasTrailer) return;
    setSmallTrailerPlaying(false);
    setIsSmallTrailerFullscreen(false);
  }, [hasTrailer]);
  ...
  const averageRating = Number(movie?.rating || 0);
  const reviewCount = Number(movie?.review_count || 0);
  const recommendedPercent = Number(movie?.recommended_percent || 0);
    ...
    <div className="movie-detail-page">
      ...
      <div className="movie-detail-content">
        ...
        <div className="movie-detail-content-inner">
          {loadingMovie && (
            <div style={{ textAlign: 'center', opacity: 0.85, padding: '16px 0' }}>
          ...
          <div className="movie-detail-hero">
            <div className="movie-detail-summary">
              <div className="movie-poster-card">
                {posterSrc ? (
                  <img src={posterSrc} alt={movie?.title || 'Movie poster'} />
                ) : (
                  <div className="trailer-waiting-poster">
                    <div className="trailer-waiting-hint">CHƯA CÓ POSTER</div>
                  </div>
                )}
              </div>
              ...
              <div className="movie-detail-info">
                ...
                <h1 className="movie-detail-title">{movieTitle || '...'}</h1>
                ...
                <div className="movie-chips-row">
                  <span><span className="icon">⏱</span>{movie?.duration ? `${movie.duration} phút` : '--'}</span>
                  <span>
                    <span className="icon">🎞</span>
                    {Array.isArray(movie?.categories) && movie.categories.length
                      ? movie.categories.map((c) => c.category_name).join(', ')
                      : '--'}
                  </span>
                  <span><span className="icon">🌍</span>{movie?.country || '--'}</span>
                  <span><span className="icon">🔞</span>{movie?.age_limit ? `${movie.age_limit}+` : '--'}</span>
                </div>
            ...
            <div className="score-card score-card-large">
              ...
              <div className="review-score-row">
                <div>
                  ...
                  <div className="score-subtitle">{formatReviewCount(reviewCount)} đánh giá</div>
                </div>
